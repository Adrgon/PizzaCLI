...
_tokens.post = function (data, callback) {
	...
	//debug(`Trying to create a token. Data:`, JSON.stringify(data));
...
_tokens.verifyToken = function (id, callback) {
	...
	if (!helpers.validateString(id)) {
		...
		_data.read(config.tokensFolder, id, function (err, tokenData) {
			if (!err && tokenData) {
				if (tokenData.expires > Date.now()) {
		...
		if (!err && tokenData) {
			if (tokenData.email == email && tokenData.expires > Date.now()) {
				...
				if (err)
					...
					//debug(`${tokenData.email == email ? 'Token is expired' : 'Email is invalid'}`);
				callback(false, null);
			}
		} else {
			callback(false, null);
